...
object ConfigParser {
    fun parse(raw: String): ClientConfig {
        ...
        require(colonRight > 0 && colonRight < right.length - 1) { "format invalid token:localPort" }
        val token = right.substring(0, colonRight)
        val localPort = parsePort(right.substring(colonRight + 1))
